...
//            .andExpect(model().attributeExists("articles"))
    ...
    @Test
    @Sql(statements = ["INSERT INTO article (name, title, contents, article_key) VALUE ('test', 'test', 'test', 'test');"])
    ...
    fun updateArticleNotMatchArticleKeyTest() {
        ...
        mockMvc.perform(
            ...
            .andExpect(status().is3xxRedirection)
            .andExpect(
                view().name("redirect:/edit/${latestArticle.id}")
            )
            .andExpect(flash().attributeExists("message"))
            .andExpect(flash().attribute("message", target.MESSAGE_ARTICLE_DOES_KEY_UNMATCH))
            .andExpect(flash().attribute("alert_class", target.ALERT_CLASS_ERROR))
    }
